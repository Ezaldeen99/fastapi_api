...
- 🎯 Entry points to execute your program using `python -m <fastapi_api>` or `$ fastapi_api` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/Ezaldeen99/fastapi_api/branch/main/graph/badge.svg?token=fastapi_api_token_here)](https://codecov.io/gh/Ezaldeen99/fastapi_api)
[![CI](https://github.com/Ezaldeen99/fastapi_api/actions/workflows/main.yml/badge.svg)](https://github.com/Ezaldeen99/fastapi_api/actions/workflows/main.yml)

Awesome fastapi_api created by Ezaldeen99

## Install

from source
```bash
git clone https://github.com/Ezaldeen99/fastapi_api fastapi_api
cd fastapi_api
make install
```
...
```bash
pip install fastapi_api
```

## Executing

```bash
$ fastapi_api run --port 8080
```

or

```bash
python -m fastapi_api run --port 8080
```

or

```bash
$ uvicorn fastapi_api:app
```
...
```bash
❯ fastapi_api --help
Usage: fastapi_api [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fastapi_api create-user --help
Usage: fastapi_api create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fastapi_api create-user admin admin --superuser
```
...
Run with `fastapi_api run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fastapi_api create-user`.
...
-----------------------------------------------------
fastapi_api/__init__.py              4      0   100%
fastapi_api/app.py                  16      1    94%
fastapi_api/cli.py                  21      0   100%
fastapi_api/config.py                5      0   100%
fastapi_api/db.py                   10      0   100%
fastapi_api/models/__init__.py       0      0   100%
fastapi_api/models/content.py       47      1    98%
fastapi_api/routes/__init__.py      11      0   100%
fastapi_api/routes/content.py       52     25    52%
fastapi_api/routes/security.py      15      1    93%
fastapi_api/routes/user.py          52     26    50%
fastapi_api/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from fastapi_api.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fastapi_api/default.toml.

### As environment variables
```bash
export fastapi_api_KEY=value
export fastapi_api_KEY="@int 42"
export fastapi_api_KEY="@jinja {{ this.db.uri }}"
export fastapi_api_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
fastapi_api_ENV=production fastapi_api run
```
